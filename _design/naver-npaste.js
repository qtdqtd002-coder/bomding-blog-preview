/*! 네이버 붙여넣기 위젯 v2 (naver-npaste.js) — 봄딩·영도 공용 캐논. 2026-09-05 (봄딩) · 09-05 영도 확대
 ...
 *    <script src="https://qtdqtd002-coder.github.io/bomding-blog-preview/_design/naver-npaste.js" defer></script>
 *    (봄딩 기존 283편은 bomding-npaste.js — 이 파일을 부르는 1줄 로더 — 로 호환)
 *  동작: 글에 이미 위젯(#npDrawer)이 있으면 서랍 안을 v2로 바꾸고(작성자 생존본 #copy는 «구 방식»으로 접어 보존),
 *        없으면 버튼·서랍을 새로 만든다. 네이버로 요청을 보내는 일은 없다 — 클립보드 복사만 한다.
 */
(function () {
  "use strict";
  ...
  function slug(s) { var v = String(s || "").replace(/[^가-힣A-Za-z0-9]/g, ""); return v ? v.slice(0, 14) : "이미지"; }
  ...
  function cleanSrc(s) {
    return String(s || "").replace(/^\s*(?:이미지\s*출처|이미지|출처)\s*[:：]\s*/, "").replace(/\s*[·|]\s*네이버.*$/, "").trim();
  }

  /* ---------- 0. 작성자·원문 ---------- */
  var WRITER = (function () {
    var path = ""; try { path = decodeURIComponent(location.pathname); } catch (e) { path = location.pathname; }
    if (/\/영도\//.test(path)) return "영도";
    if (/\/봄딩\//.test(path)) return "봄딩";
    var head = txt(document.querySelector(".topbar")) + " " + txt(document.querySelector(".meta"));
    if (/kkodug9|영도|영원한도시/.test(head)) return "영도";
    return "봄딩";
  })();
  var YD = WRITER === "영도";
  var POST = document.querySelector(".post") || document.querySelector(".post-content") || document.querySelector(".wrap") || document.body;
  var TITLE = txt(document.querySelector("h1.title")) || txt(document.querySelector(".post-title")) || (document.title || "").trim();
  var TAGS = (function () {
    var t = document.querySelector(".tags, .tags-line"); var s = txt(t);
    var m = s.match(/#[^\s#,]+/g); return m ? m.join(" ") : s.replace(/^태그\s*[:：]\s*/, "");
  })();

  /* 미리보기 전용(네이버에 안 들어가는) 요소 — 통째로 건너뛴다. .chk 는 작성자별 분기(cleanNode) */
  var SKIP = ".topbar,.cat,h1.title,.post-title,.post-meta,.meta,.tags,.pvbar,.sidenote,.src,.src2,.src-line,.cap,.shoot,.imgsrc,.frame,.paste," +
  ...
  function cleanNode(c, into) {
    ...
    if (hasCls(c, "chk")) {
      if (!YD) return;   /* 봄딩: 미리보기 전용 마커 제거(§3) */
      var t = txt(c).replace(/🔍/g, "").replace(/^\s*(?:확인\s*[:：]\s*)?/, "").trim();   /* 영도: 문장 안에 박힌 경우가 있어 파란 «(확인: …)»으로 남긴다 */
      var sp = document.createElement("span"); sp.className = "ph"; sp.textContent = " (확인: " + (t || "직접 확인") + ")";
      into.appendChild(sp); CHKS++; return;
    }
    var tag = c.tagName;
    if (tag === "IMG") return;
  ...
  Builder.prototype.shot = function (ss) {
    ...
    var meta = this.metaLinks(nextSib(ss, ".shoot"), "비슷한 화면 찾을 곳:");
    if (YD) {   /* 영도 §4-2: 촬영박스는 실게시 본문에 안 들어간다 — 서랍엔 회색 안내만(복사 제외) */
      this.flush();
      var w = el("div", "np2-spot np2-viewonly");
      w.appendChild(el("div", "np2-meta", "📷 촬영 안내(복사엔 안 들어감): " + esc(lbl || "스크린샷")));
      if (meta) w.appendChild(meta);
      this.out.appendChild(w); return;
    }
    this.spot(lbl || "스크린샷", meta, "shoot");
  };
  ...
  Builder.prototype.image = function (img, wrap) {
    ...
    if (srcLine) {
      ...
      srcName = txt(a) || cleanSrc(txt(srcLine));
      srcUrl = a ? abs(a.getAttribute("href")) : "";
    }
    var label = alt || cap || ("사진 " + n);
    var base = (src.split("?")[0].split("/").pop() || "image").replace(/%[0-9A-Fa-f]{2}/g, "");
    /* 영도 §4-2 «출처 줄 동반 이관»: .src 가 붙은(Tier 2) 이미지는 사진 아래 출처 한 줄을 같이 낸다 */
    var attr = (YD && srcName) ? ("이미지: " + cleanSrc(txt(srcLine)) ) : "";
    if (attr && !/제공|출처|©/.test(attr)) attr += " 제공";
    var it = { n: n, url: src, alt: label, srcName: srcName, srcUrl: srcUrl, attr: attr, file: pad2(n) + "_" + slug(srcName || WRITER) + "_" + safeName(base) };
    this.items.push(it);
    ...
    fig.innerHTML = '<img class="np2-thumb" src="' + esc(src) + '" alt="' + esc(label) + '" loading="lazy">' +
      ...
      "</div>" +
      (attr ? '<p class="np2-attr">' + esc(attr) + "</p>" : "") +
  ...
  Builder.prototype.igrow = function (r) {   /* 영도 인포그래픽 막대(.ig-row) → «라벨: 값» */
    var lb = txt(r.querySelector(".ig-lb")), v = txt(r.querySelector(".ig-val"));
    var p = document.createElement("p");
    if (lb) { var b = document.createElement("b"); b.textContent = lb; p.appendChild(b); p.appendChild(document.createTextNode(v ? ": " + v : "")); }
    else p.textContent = txt(r);
    if (txt(p)) this.block(p);
  };
  Builder.prototype.walk = function (node) {
    for (var c = node.firstChild; c; c = c.nextSibling) {
      ...
      if (hasCls(c, "ig-row")) { this.igrow(c); continue; }
      ...
      if (tag === "H4" || tag === "H5" || tag === "H6" || tag === "SUMMARY" || hasCls(c, "bt") || hasCls(c, "vh") || hasCls(c, "sn-h") ||
        hasCls(c, "ig-h") || hasCls(c, "qq") || hasCls(c, "faq-q") || (hasCls(c, "h") && tag !== "TD" && tag !== "TH")) { this.boldLine(c); continue; }
  ...
  Builder.prototype.finish = function () {
    ...
    if (YD && TAGS) out.appendChild(el("p", "np2-tags", esc(TAGS)));   /* 영도 §4-2 «유지»: 태그 줄은 본문 끝에 */
    return out;
  };
  ...
  var AUTO = B.finish();
  var ITEMS = B.items;
  var STATS = { writer: WRITER, photos: B.photo, imgs: B.imgs, spots: B.spots, coupang: B.coupang, tables: B.tables, btns: B.btns, heads: B.heads, chks: CHKS, blocks: AUTO.children.length };

  /* ---------- 2. 복사용 HTML 만들기 ---------- */
  function buildCopyNode(root, imgMode, headMode) {
    var c = root.cloneNode(true);
    [].forEach.call(c.querySelectorAll(".np2-ui,.np2-meta,.np2-thumb,.np2-viewonly"), function (n) { n.remove(); });
    [].forEach.call(c.querySelectorAll(".np2-fig"), function (f) {
      ...
      if (it.attr) { var ap = document.createElement("p"); ap.textContent = it.attr; p.parentNode.insertBefore(ap, p.nextSibling); }
    });
  ...
  var CSS = "" +
    ...
    ".np2-copy .np2-tags{color:#5a8;}.np2-copy .np2-attr{font-size:13px;color:#666;margin:0 0 10px;}" +
  ...
  function statLine() {
    ...
    if (STATS.chks) s.push("확인 표시 " + STATS.chks);
    return s.join(" · ");
  }
  function guideHtml() {
    ...
    var k = ITEMS.length ? 4 : 3;
    var blue = [];
    if (STATS.spots) blue.push("📷 사진");
    if (STATS.coupang) blue.push("👉 쿠팡 링크");
    if (STATS.chks) blue.push("(확인: …) 표시는 확인 후 지우기");
    if (blue.length) { g.push("<b>" + circ(k) + "</b> <b>파란 글씨</b>는 직접 채우기 — " + blue.join(" · ")); k++; }
    if (TAGS) g.push("<b>" + circ(k) + "</b> [태그 복사] → 태그칸" + (YD ? " (본문 끝 태그 줄은 필요 없으면 지우기)" : ""));
    return g.join("<br>");
  }
  ...
  window.__np = {
    ...
    writer: WRITER, title: TITLE, tags: TAGS, htmlText: htmlText,
    autoText: function () { return htmlText(AUTO); },
    articleText: function () {
      var c = POST.cloneNode(true); [].forEach.call(c.querySelectorAll(SKIP_ART), function (n) { n.remove(); }); return htmlText(c);
    },
