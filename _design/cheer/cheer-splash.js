/* ══════════════════════════════════════════════════════════════════
   쓰담 — 축하 스플래시 「봄딩 · 네이버 메이트 2026.09」  (2026-09-04 · v2 «빵빠레» 09-05)

   무엇  : 사이트에 «접속»했을 때 약 3.5초, 봄딩의 네이버 메이트 선정을 축하하는 화면을 띄운 뒤
   ...
   v2    : ① 영도가 플라크 «옆»을 타원 궤도로 날아다니며 축하(앞을 지날 땐 크게·뒤를 지날 땐 작게, 진행 방향으로 기울고
           왼쪽으로 갈 땐 좌우 반전, 두 포즈를 0.4초마다 교대 = 날갯짓, 꼬리 반짝이, «축하해요!» 말풍선)
           ② 봄딩은 «기뻐서 뛰기» 루프(앤티시페이션 스쿼시 → 점프+포즈 교체 → 착지 스쿼시 + 하트 팝)
           ③ FX 빵빠레: 코너 캐논 2발(컨페티+스트리머+머즐 플래시) → 화면 플래시+충격파 링+방사 버스트+플라크 킥(팡파르)
              → 불꽃 4발(잔상 스파크) → 상시 드리즐 컨페티 + 트윙클. 캔버스 2겹(뒤/앞) — 앞 겹은 플라크 위로도 떨어진다.
   모션  : GSAP(사이트가 _vendor/gsap 로 이미 벤더)이 있으면 타임라인, 없으면 같은 안무의 CSS 키프레임 폴백(FX 는 자체 rAF 라 폴백에서도 터진다).
           ...
           CSS 애니메이션(cheerDead)이 6초에 강제로 숨긴다. 스플래시는 화면을 통째로 가리므로 걷히지 않는 실패 모드 = 서비스 중단이다.
   FX    : 의존성 없는 Canvas 2D 파티클(컨페티 리본·원·하트·별 / 스파크(잔상선) / 스트리머(물결 폴리라인) / 링 / 트윙클 / 플래시선). DOM 노드 0개, 상한 900.
   ...
   규칙  : 색은 정체성에만(봄딩 --w-bomding · 영도 --w-yeongdo · 네이버 그린) · 이중 베젤 · Pretendard 800 · 라벨 ≥12.5px ·
           바운스/엘라스틱 이징 없음(펀치는 «크게서 작게 expo.out»으로) · transform/opacity 위주(조리개 mask 1회만 예외) ·
...
var CFG={
  ...
  hold:2500,                    /* 안무 시작 후 최소 노출(ms). 이 뒤 «데이터 준비»가 되면 걷힌다(총 ≈3.5초) */
  maxWait:4300,                 /* 하드 백스톱(ms, 스크립트 시작 기준): 데이터가 영영 안 와도 걷힌다 */
...
'  animation:cheerDead 1ms linear 6s forwards}',                                                        /* JS 가 죽어도 반드시 걷힌다 */
...
'.cheer.css .cheer-badge{animation:cheerUp .8s var(--e) .1s both,cheerHop 1.15s var(--e) 1.4s infinite}',
...
'.cheer.css .cheer-shine i{animation:cheerShine .8s cubic-bezier(.45,0,.2,1) .55s both}',
'.cheer.css .cheer-fly{animation:cheerFade .3s var(--e) .9s both,cheerOrbit 2.9s linear .9s infinite}',
'.cheer.css .cheer-say{animation:cheerSay 1.3s var(--e) 1.55s both}',
'.cheer.css .cheer-flash{animation:cheerFlash .34s var(--e) .78s both}',
'.cheer.css.out{animation:cheerOut .5s var(--e) forwards,cheerDead 1ms linear 6s forwards}',
...
'@keyframes cheerFlash{0%{opacity:0}18%{opacity:.5}100%{opacity:0}}',
...
var t0=performance.now(), gsapRef=null, tl=null, hopTl=null, orbTw=null, sayTl=null, playedAt=0, ready=false, gone=false, exiting=false, timers=[], DBG=[];
...
var FX=(function(){
  var cb=$('cheerFxB'), cf=$('cheerFxF'), xb=cb.getContext('2d'), xf=cf.getContext('2d');
  var P=[], W=0, H=0, dpr=1, raf=0, last=0, on=false, drizzle=false, frame=0, CAP=900;
  ...
  function size(){
    dpr=Math.min(2,window.devicePixelRatio||1); W=cb.clientWidth; H=cb.clientHeight;
    [cb,cf].forEach(function(c){ c.width=Math.round(W*dpr); c.height=Math.round(H*dpr); });
  ...
  function conf(x,y,ang,spread,speed,layer,colors,o){
    o=o||{}; var a=ang+(Math.random()-.5)*spread, s=speed*(.5+Math.random()*.75), r=Math.random();
    add({k:0,x:x,y:y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,
      ...
      tilt:Math.random()*6.28,vt:.12+Math.random()*.2,c:pick(colors||C),
  ...
  function cannons(){
    start();
    var m=mobile(), n=m?80:150, sp=m?16.5:21, cx1=W*.08, cx2=W*.92, y=H+8;
    ...
    for(var s=0;s<(m?4:7);s++){ streamer(cx1,y,-Math.PI/2+.42,.7,sp*.75,1); streamer(cx2,y,-Math.PI/2-.42,.7,sp*.75,1); }
    muzzle(cx1,H-6,-Math.PI/2+.46); muzzle(cx2,H-6,-Math.PI/2-.46);
    drizzle=true;
  }
  function muzzle(x,y,ang){
    ring(x,y,70,'#FFFFFF',1,.35,5);
    for(var i=0;i<9;i++)flashline(x,y,ang+(i-4)*.16,10,rnd(40,90),pick(['#FFFFFF','#F3C4D0','#8FE3B3']),1);
  }
  /* 팡파르 — 화면 중앙(스티커)에서 사방으로 */
  function fanfare(x,y){
    start();
    var m=mobile(), n=m?60:110;
    for(var i=0;i<n;i++)conf(x,y,Math.random()*6.28,0,m?8:11,1,C,{g:.16,decay:.008});
  ...
  function firework(x,y){
    ...
    for(var i2=0;i2<(m?14:22);i2++){ var a2=Math.random()*6.28, s2=rnd(1.2,3.4); spark(x,y,Math.cos(a2)*s2,Math.sin(a2)*s2,'#FFFFFF',1,{decay:rnd(.02,.03),lw:2}); }   /* 중심 화이트 코어 */
...
var ORB={a:Math.PI, t:0, front:null};
function placeFly(){
  ...
  g.set(fly,{x:x,y:y,rotation:ang,scaleX:mir?-sc:sc,scaleY:sc});
  if(hasFlyB){ var ph=Math.floor(ORB.t/.4)%2; fly.classList.toggle('b',ph===1); }
  /* 말풍선은 영도 머리 옆(진행 방향 쪽) */
...
function playG(g){
  ...
  g.set(badge,{transformOrigin:'50% 100%'});
  ...
  tl.fromTo(tray,{opacity:0,y:28,scale:.97},{opacity:1,y:0,scale:1,duration:.85},0)
    ...
    .add(function(){ FX.cannons(); },.55)
    .fromTo(shine,{xPercent:-130,x:0},{xPercent:130,duration:.8,ease:'power2.inOut'},.55)
    ...
    /* ★팡파르 — 플래시 + 충격파 + 방사 버스트 + 플라크 킥 */
    .add(function(){ var c=badgeCenter(); FX.fanfare(c.x,c.y); },.78)
    .fromTo(flash,{opacity:0},{opacity:.5,duration:.06,ease:'power1.out'},.78)
    .to(flash,{opacity:0,duration:.3,ease:'power2.out'},.84)
    .fromTo(tray,{scale:1.035},{scale:1,duration:.55,ease:E,immediateRender:false},.8)
    .fromTo(sub,{opacity:0,y:8},{opacity:1,y:0,duration:.5},.95)
    /* 영도 등장 → 궤도 비행 */
    .add(function(){
      ORB.a=Math.PI; ORB.t=0; ORB.front=null; placeFly();
      orbTw=g.to(ORB,{a:Math.PI-6.28*4,t:2.9*4,duration:2.9*4,ease:'none',onUpdate:placeFly});
    },.9)
    .fromTo(fly,{opacity:0},{opacity:1,duration:.32,ease:'power2.out'},.9)
    /* 봄딩 — 기뻐서 뛰기 루프(앤티시페이션 → 점프+포즈 B → 착지 스쿼시 + 하트) */
    .add(function(){
      hopTl=g.timeline({repeat:-1,repeatDelay:.32});
      hopTl.to(badge,{scaleY:.93,scaleX:1.06,duration:.13,ease:'power2.in'})
    ...
    /* 불꽃 4발 */
    .add(function(){ FX.firework(FX.W()*.2,FX.H()*.22); },1.25)
    .add(function(){ FX.firework(FX.W()*.8,FX.H()*.2); },1.7)
    .add(function(){ FX.firework(FX.W()*.5,FX.H()*.1); },2.15)
    .add(function(){ FX.firework(FX.W()*.32,FX.H()*.3); },2.55)
    /* 말풍선 */
    .add(function(){
      sayTl=g.timeline();
      sayTl.fromTo(say,{opacity:0,scale:.8},{opacity:1,scale:1,duration:.32,ease:E})
           .to(say,{opacity:0,scale:.92,duration:.22,ease:'power2.in'},1.35);
    },1.55);
  root.classList.remove('pre');   /* fromTo 가 시작값을 inline 으로 잡은 뒤라 깜빡임 없음 */
}
function playCss(){
  root.classList.add('css'); root.classList.remove('pre');
  later(FX.cannons,550);
  later(function(){ var c=badgeCenter(); FX.fanfare(c.x,c.y); },780);
  later(function(){ FX.firework(FX.W()*.2,FX.H()*.22); },1250);
  later(function(){ FX.firework(FX.W()*.8,FX.H()*.2); },1700);
  later(function(){ FX.firework(FX.W()*.5,FX.H()*.1); },2150);
  if(hasFlyB)later(function(){ var iv=setInterval(function(){ if(gone){ clearInterval(iv); return; } fly.classList.toggle('b'); },400); timers.push(iv); },900);
}
function play(){
  if(gone)return;
...
/* 종료: 영도가 오른쪽 위로 날아가고(0.38s) → 판이 물러나고(0.3s) → 조리개가 열린다(0.62s). mask 미지원이면 페이드 */
function exit(){
  if(gone||exiting)return; exiting=true;
  try{
    if(orbTw)orbTw.kill(); if(hopTl)hopTl.kill(); if(sayTl)sayTl.kill();
    ...
    if(gsapRef){
      ...
      t.to(fly,{x:'+=560',y:'-=200',rotation:-16,opacity:0,duration:.38,ease:'power2.in'},0)
       ...
       .to(stage,{opacity:0,y:-12,scale:.975,duration:.3,ease:'power2.in'},.08)
       .to([skip,$('cheerFxB'),$('cheerFxF')],{opacity:0,duration:.25,ease:'power2.in'},.08);
      if(maskOK){
...
function teardown(){
  ...
  try{ if(tl)tl.kill(); if(orbTw)orbTw.kill(); if(hopTl)hopTl.kill(); if(sayTl)sayTl.kill(); }catch(e){}
  FX.stop();
...
function watchReady(){
  ...
  try{
    var mo=new MutationObserver(function(){
      if(!view.querySelector('.skel')){ mo.disconnect(); dbg('data'); later(function(){ ready=true; dbg('ready'); maybeExit(); },380); }   /* 사이트 첫 렌더 버스트(행 170·차트·FLIP)가 rAF 를 0.4초쯤 붙드니 그 뒤에 퇴장 */
    });
