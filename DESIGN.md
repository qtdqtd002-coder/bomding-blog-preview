...
### Cheer Splash — 축하 스플래시 (2026-09-04 · v2 «빵빠레» 09-05 · 한시 운영 · `_design/cheer/`)
봄딩의 네이버 메이트 선정(2026-09)을 축하하는 진입 화면. v5 가 폐기한 «부팅 스플래시»의 부활이 아니라 **한시 이벤트 레이어**다 —
`index.html` 은 `<body>` 첫 줄의 `<script src="_design/cheer/cheer-splash.js">` 한 줄만 알고, 파일 하나가 스타일·마크업·모션·컨페티를 스스로 넣는다(걷어낼 땐 그 한 줄 삭제).

- **구성**: 이중 베젤 플라크(트레이 24/코어 18) 위쪽 가장자리에 봄딩 스티커(나노바나나 — 봄딩 아바타 `-Ref` → 평면 배경 flood-fill 키잉·트림, webp 30KB+png 폴백)가 걸쳐 얹히고,
  네이버 그린 점 키커 «네이버 메이트 · 2026년 9월» → 44px/800 헤드라인(«봄딩» 만 `--w-bomding`) → «당신의 글쓰기 동료, 쓰담 드림». 색은 정체성에만(봄딩 핑크·네이버 그린) — v5 원칙 그대로.
- **안무(GSAP, 없으면 같은 안무의 CSS 키프레임)**: 플라크·스티커 떠오름(expo.out) → 키커 → 글자별 스태거(어절 inline-block 으로 keep-all 유지, 총 .42s)+blur 8→0 → 컨페티 2발(Canvas 2D 파티클 — 리본·원·하트·별, 중력·항력·플러터, DOM 0)
  → 스티커 실루엣 마스크 **아래로** 광택 띠 1회(마스크는 고정, 안의 띠만 이동 — 마스크와 함께 움직이면 실루엣 잔상이 화면 밖에 뜬다) → 스티커 미세 부유. 바운스·엘라스틱 없음.
- **종료**: 최소 노출 2.0s + «데이터 준비»(#view 스켈레톤 소멸) 뒤 플라크가 물러나고(0.3s power2.in) 조리개(`mask-image: radial-gradient`, 36px 페더, `--r` 을 GSAP 이 tween)가 열린다(0.62s). 총 ≈3초. 클릭·Esc·Enter·Space·우하단 «건너뛰기»(36px 알약) 로 즉시 종료.
- **★반드시 걷힌다**: maxWait 4.0s 백스톱 + JS 가 죽어도 CSS `cheerDead` 가 5.6s 에 강제 숨김. 가짜 진행률 없음(v4 §Boot 원칙 계승).
- **빈도·수명**: 세션당 1회(`sessionStorage.sseudam_cheer_202609`) · `?cheer=1` 재생 · `?cheer=0` 끔 · **`CFG.until` 2026-10-04 지나면 자동 종료** — 연장·단축은 상수 하나. 모션은 항상 켬(§Motion 결정과 동일).
- **v2 «빵빠레»(09-05, 사용자 «영도가 옆에서 날아다니며 축하·캐릭터 모션·FX 더 빵빵»)**:
  ① **영도**(`yeongdo-fly-a/b`, 아바타 -Ref 비행 포즈 2장) 가 플라크 중심 타원 궤도(rx=폭/2+96·ry=높이/2+64, 한 바퀴 2.9s)를 돈다 — 왼쪽에서 «앞(아래)»으로 먼저 지나며
  앞을 지날 땐 z 3·scale 1.0, 뒤(위)를 지날 땐 z 0(트레이 뒤)·scale .78, 진행 방향으로 기울고(±28° 클램프) 왼쪽으로 갈 땐 좌우 반전, 두 포즈를 0.4s 마다 교대(날갯짓, 90ms 크로스페이드),
  꼬리에 틸 반짝이, 1.55s 에 «축하해요!» 말풍선(영도 점 `--w-yeongdo`) 1.3s. 퇴장 땐 오른쪽 위로 날아간다.
  ② **봄딩 «기뻐서 뛰기» 루프**(1.4s 부터, 1.15s 주기): 앤티시페이션 스쿼시(scale 1.06/.93) → 점프 −28px + 포즈 교체(`bomding-jump`, 두 팔 위로) → 착지 스쿼시 + 하트 팝(앞 캔버스). transform-origin 하단 중앙.
  ③ **FX**: 캔버스 2겹(뒤/앞 — 앞 겹은 플라크 위로도 떨어진다). 0.55s 코너 캐논 2발(각 150 + 스트리머 7 + 머즐 플래시 링·선) → 0.78s **팡파르**(화면 화이트 플래시 .5 → 0, 충격파 링 2(핑크 280·그린 200), 방사 컨페티 110, 플래시선 14, 트윙클 14, 플라크 킥 scale 1.035→1) → 불꽃 4발(1.25·1.7·2.15·2.55s, 잔상 스파크 84 + 화이트 코어 + 링 2) → 상시 드리즐(2프레임당 1). 파티클 상한 900, 모바일 ≈55%.
  ④ **포즈 쌍 정렬**: 키잉·트림한 두 장을 같은 캔버스에 하단 중앙 정렬(`pair_align.py`) — 안 맞추면 교체 순간 캐릭터가 튄다. B 파일이 없으면 `onerror` 로 A 한 장 모션만.
  ⑤ **퇴장 지연 380ms**: 데이터 도착 직후 사이트의 첫 렌더 버스트(행 170·차트·FLIP)가 rAF 를 0.4초쯤 붙들어 GSAP 시계가 밀린다 → 스켈레톤 소멸 뒤 380ms 지나서 «준비»로 친다(그 사이는 스플래시가 가려 준다). 총 ≈3.5초(hold 2.5s), maxWait 4.3s, CSS 백스톱 6s.
- **검증법**: 헤드리스 크롬 `--virtual-time-budget` 캡처는 **GSAP 안무를 진행시키지 못한다**(빈 플라크만 찍힘) — CDP 실시간 프로브(`Runtime.evaluate` 로 opacity·클래스·`--r` 읽기 + `Page.captureScreenshot`)로 데스크톱 1920·모바일 390·GSAP 차단 폴백·클릭 스킵·세션 1회·`?cheer=0` 을 확인했다.
