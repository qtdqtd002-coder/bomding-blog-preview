...
| **사용량** (7) — 09-05 통합 | 토큰=`_trend/_usage.json`(하루 1회 스냅샷·추정) · 편수=`/requests` 종결 상태(실시간·1분 재조회) | 스탯 4칸(오늘 발행·7일 발행·7일 실패율·7일 토큰) → 14일 **한 차트**: 작성자 스택 막대(토큰 축 하나, ≤24px·2px 간격·헤어라인)+막대 아래 **발행 편수 행**(`.pubn`, 실패는 `·N` 경고색, 오늘은 잉크). 이중 축 금지라 편수는 축이 아니라 «행». 캡 라벨=최대·오늘만, 범례=작성자+발행+실패, 툴팁 한 개에 토큰·편수 전부, 표 뷰. 미연결이면 편수 칸 «미연결»(0 아님) |
...
| ~~잔여 사용량~~ → **상단 고정 칩 `.qchip`** — 09-05 | `_trend/_quota.json`(`_tools/quota.cjs`) | 종(`.bell`) 왼쪽 `right:74px` 44px 유리 칩, 모든 탭에서 보임: 게이지 아이콘 + «남음» + `5시간 72%` · `7일 87%` 둘뿐. 남음 ≤15% 면 그 숫자만 경고색. 상세(모델별 7일·초기화 시각·스냅샷 시각·플랜)는 hover·focus 툴팁(`.ctip`, 화면 안으로 접힘). 스냅샷 없으면 칩을 숨긴다(0 위장 금지). 모바일(≤860)은 섬이 전폭이라 `top:66px right:12px` 34px |
...
- **색 검증(09-05 추가)**: 봄딩 `#C43D63` ↔ 영도 `#0F7C86` 쌍 — CVD ΔE 8.7·정상시 26.0·대비 ≥3:1 PASS, 영도 채도(0.089) 만 «회색으로 읽힘» FAIL → 정체성 색이라 유지하고 2차 인코딩(끝 라벨·범례·툴팁·표 뷰)으로 보완. «한 축» 규칙: 「사용량」은 토큰과 발행 편수의 단위가 달라 편수를 **축이 아니라 막대 아래 행**으로 넣어 지켰다(이중 축 금지).
