...
- **썸네일(봄딩 · 정사각 1000)** `_toolbox/thumb.js` + 미리보기 자산 `_toolbox/thumb/`(≈140KB). 규격 = **봄딩 썸네일 표준 v1**(2026-09-05, `쓰담/docs/…/썸네일표준/apply-thumb-standard.py` 를 그대로 이식) — 1000×1000 · 글자 높이 16.5% · 안전여백 4% · 테두리 없음 · **부제 없음이 기본**(켜면 «90px에서 읽히지 않음» 경고를 검사 목록과 입력 밑 두 곳에). 프리셋 5 = **밴드(표준)** · 플라크 · 틴트 · 스트라이프 · 크림(구조만 다르고 글자 높이·여백은 같다) · 포인트 색 5(시안 A 팔레트: 로즈·딥 로즈·라벤더·민트·골드, 스트라이프 띠 글자는 띠 밝기 L≥.25 면 플럼) · 글꼴 2(Pretendard 800 · 맑은 고딕). 이미지 = 드롭·클릭·**Ctrl+V**, 확대 1~3×(슬라이더·휠·+/−)·드래그/화살표 이동·가운데. 왼쪽 판 = 캔버스(1000, 표시 ≤400px) + **실제 크기 미니 2**(메인 목록 183×185 · 프롤로그 90×90, 절반씩 단계 축소) + **검사 3**(원본 해상도 ≥1000 · 타이틀 ≤6자 · 부제 없음). 내보내기 = **PNG 저장**(`<타이틀>_1000.png`) · **클립보드 복사**(네이버 에디터에 붙여넣기). 설정(프리셋·색·글꼴·부제)은 `localStorage.sseudam_thumb_v1`, 이미지는 저장하지 않는다.
...
- **게이트** `_tools/verify-toolbox.mjs`(헤드리스 CDP · 의존성 0 · 64항목: 탭 진입·로드·이미지 등록·타이틀 카운터·프리셋 5·긴 제목 축소·부제 경고·색·글꼴·확대·드래그·가운데·PNG 파일명·blob·미리보기 실크기 183/90/모바일 3열·Esc·포커스 복귀·탭 이탈/복귀 상태 유지·390/360 섬 넘침·종 겹침·모달 축소 배율) + design-reviewer 실렌더.
