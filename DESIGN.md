...
탭 순서 **홈 · 작성 글 · 트렌드 · 도구함**. 좌측 **도구 목록**(232px 트레이, 행 = **아이콘 타일 + 이름**뿐 — 한 줄 부제는 09-06 사용자 지시로 폐기(§Do/Don't 설명문 금지), 선택 = 잉크 채움 — 탭 알약·칩과 같은 어휘) + 우측 **도구 판**(트레이 코어, `min-height:440px`). 860px 이하는 목록이 가로 한 줄(스크롤)로 접힌다.
...
- **티어표 제작**(2026-09-07 신설 · v1 「잉크 농도 보드」) `_toolbox/tier.js`(의존성 0·자산 0) — 블로그 본문용 티어표 **PNG(폭 1000 · 높이 자동)**. 사용자 요구 = «흔히 보는 티어표가 아닌 독창적이고 직관적인 쓰담 전용 양식, 나중에 게임을 고르면 캐릭터 일러스트 크롭이 꽂히는 자리». **흔한 티어표와 다른 점 6**: ①**농도 = 등급** — 무지개 색띠(S빨강·A주황·B노랑…) 대신 포인트 색 하나를 `1.0 → 0.14` 등간으로 옅혀 플라크를 채운다(흰 바탕에 실색으로 섞어 만들고, 그 색의 휘도 <.42 면 흰 글자·아니면 잉크 — 범례 없이 «진할수록 강하다») ②**초상 타일 + 이름** 카드(이름 없는 아이콘 나열 금지 — 얼굴을 몰라도 읽힌다) ③**전체 순위 배지**(1…N, 잉크 원+흰 테두리, 끌 수 있음) ④**티어별 «한 줄»**(왜 이 등급인가, 비우면 안 그림) + **바닥 기준 스탬프·서명**(«v1.2 패치 · 9.7 기준» / «● 봄딩» — 검증을 보여 준다) ⑤**세로 보드**(독자 대다수가 모바일 — 가로 띠 티어표는 폰에서 안 읽힌다; 검사 목록이 «폰에서 이름 ≈ 11.9px» 를 1000→350 환산으로 보여 주고 10px 미만이면 경고) ⑥**액자 = 사이트와 같은 이중 베젤**(트레이 28/코어 18, `:root` 토큰을 마운트 때 읽어 캔버스에 씀). 이미지 없으면 **모노그램 타일**(이름 첫 글자), 이름도 없으면 **점선 빈 슬롯**(= 양식 자체). 치수: 트레이 10 · 코어 패딩 36 · 플라크 112 · 타일 = (770 − 간격)/열 → 3열 244 / 4열 179 / 5열 139 / 6열 113, 이름 = 타일×.19(26~40) · 제목 44/800 2줄까지 · 부제 24 · 티어 글자 62/50/40(1/2/3자). **편집** — 왼쪽 캔버스(보드 자체): **끌어서 티어·순서 변경**(마우스·펜, 고스트+잉크 삽입 마커, 놓을 자리 = 띠(y)→줄(y)→줄에서 중심이 포인터 오른쪽인 첫 타일 앞) · 클릭 = 선택(화면에만 링, 내보내기엔 없음) · Esc 해제 · **터치는 탭=선택만**(`touch-action:pan-y` — 긴 보드 위에서 페이지 세로 스크롤을 뺏지 않는다, 이동은 편집 열 ▲▼·티어 셀렉트) · 파일 드롭(타일 위=그 항목 그림 · 띠 위=그 티어에 새 항목, 파일명이 이름 · 여러 장=이어서) · Ctrl+V(선택 항목에, 없으면 «넣을 티어»에 새 항목) · JSON 드롭=불러오기. 오른쪽 편집 열: 보드(제목 40자 카운터·부제·기준·서명) / 티어(프리셋 3 = S~D 5단계·S~C 4단계·1~3군 → 글자·라벨만 바꾸고 항목은 자리 순서대로 남기며 줄어든 티어의 항목은 마지막 티어로 · 행 = 농도 점·글자(3자)·라벨(8자)·한 줄(30자)·삭제(항목은 옆 티어로) · 추가 최대 7) / 항목(«넣을 티어» 셀렉트 + 이름 입력 **쉼표로 여러 개** + Enter · 행 = 썸네일 버튼(이미지 넣기)·이름(14자)·티어 셀렉트(글자만 — 62px 칸에 «S · 최상위»는 «S · 최싱»으로 잘렸다)·▲▼(**티어 경계를 넘는다**: 첫 항목 ▲ = 앞 티어 끝, 끝 항목 ▼ = 다음 티어 처음)·이미지 지우기(있을 때만)·삭제, 최대 200) / 디자인(포인트 색 = 썸네일 도구와 같은 봄딩 3·영도 3·기타 팔레트 + 농도 띠 · 한 줄 3~6개 칩 · 순위 번호·이름 토글 칩 · 타일 둥근 사각/원). **삭제·프리셋·비우기·불러오기는 전부 토스트 «되돌리기»**(내용 스냅샷 1장, 이미지는 id 로 이어져 같이 돌아온다). 내보내기(캔버스 아래) = 클립보드 복사 · 보드 JSON(`<제목>_board.json`, 이미지 미포함) · 불러오기 · 비우기 · **PNG 저장 `<제목>_1000x<H>.png`**. 저장 = 내용·디자인 `localStorage.sseudam_tier_v1`, **이미지는 모듈 메모리**(탭 전환엔 남고 새로고침엔 사라진다 — 양식 단계의 의도적 한계, 캐릭터 크롭 파이프라인이 붙을 때 자산 저장을 정한다). 검사 목록 = 항목·티어 수 / 이미지 없는 항목(전부면 «양식 상태») / 빈 슬롯 / 폰 이름 크기 / 크기(높이 3200 넘으면 «나눠 올리기»). 게이트 `_tools/verify-tier.mjs`(83항목: 로드·기본 보드·문구·파일명·쉼표 추가·모노그램·▲▼ 경계·셀렉트 이동·**실제 마우스 드래그**·클릭 선택·프리셋+되돌리기·티어 추가/삭제·한 줄/순위/이름/모양·색 실측·이미지 등록/지우기·PNG blob·내보내기에 선택 링 없음·삭제 되돌리기·**아이콘 버튼 히트존(±6) 상호 겹침 0·칩 32px·빈 티어 자리 클릭=슬롯 추가**·12.5px 하한·포커스 링·탭 이탈/복귀·390 단일 열/넘침/pan-y/터치 타깃) + design-reviewer 실렌더(09-07 1차 🔴0·🟡3 → 반영 후 재측정 PASS: ①`.tier-it` 아이콘 4개가 gap 4px 라 ±6 히트존이 8px 겹쳐 «up 안쪽 1px 클릭이 down 으로» → `.tier-ibs` 무리 gap 12(히트존이 정확히 맞닿음)·티어 행 gap 8 ②칩 30px 별도 스케일 → 사이트 기본 32px ③항목 0개 티어의 점선 «+» 가 눌러도 무반응 → 누르면 그 티어에 슬롯 추가·커서 pointer) — ★하네스 교훈: 편집 열의 `scrollIntoView` 가 캔버스를 뷰포트 밖으로 밀어 두므로 캔버스에 마우스를 보내기 전에 그 캔버스 y 가 화면 가운데 오게 먼저 스크롤한다(안 하면 허공 클릭 = «드래그 실패»로 오판).
