...
| **AI 브리핑 인용** (7) — 09-05 | `_trend/_citations.json`(`_tools/mate-citations.cjs`, 네이버 공개 프로필 일별 스냅샷) | 작성자 행(`.cr`, 헤어라인 구분·카드 속 카드 금지): 점+이름+«9월 게임 메이트» 칩 → 스탯 4칸(어제 증분·이달·기준월·누적, 모노 숫자) → 14일 단일 시리즈 막대(작성자 색, ≤24px, 캡 라벨=최대·마지막만, 툴팁=증분·누적·스냅샷 시각, 표 뷰). **어제까지만 값이 있고 오늘은 «집계 전»**. 인용수는 메이트 선정 블로그만 공개라 영도는 «인용수 비공개 · 메이트 선정 시 자동 공개»(0 으로 그리지 않는다) |
| **일별 방문자** (5) — 09-05 | 같은 파일의 `visitors`(영도=공개 카운터 XML 달력일 · 봄딩=누적 방문자 06:00 차분) | **한 축**에 선 2개(작성자 색 2px, 이중 축 금지). 끝점 8px+표면 링·끝 라벨(잉크, 겹치면 13px 밀기)·범례(선 키+최근값)·날짜 라벨은 3일 간격·hover=수직 헤어라인+점(≥8px)+«한 툴팁에 두 값»·focus 동일·표 뷰. 결측일은 선을 끊는다(0 아님) |

- **과도기 상태(09-05)**: 두 타일은 «한쪽만 있음·전무»를 정상 상태로 그린다 — 인용 증분이 없으면 스탯 4칸(네이버 원값)만 두고 «일별 증분 집계 전 · 스냅샷 N개», 인용수 비공개(메이트 선정 전)면 «인용수 비공개 · 메이트 선정 시 자동 공개» 1줄, 방문자는 값 있는 작성자 선만 그리고 없는 날은 선을 끊는다. 빈 자리를 0이나 장식으로 채우지 않는다. 키보드 포커스 툴팁은 열 «옆»에 붙는다(위에 올리면 타일 헤더와 겹침).
- **색 검증(09-05 추가)**: 봄딩 `#C43D63` ↔ 영도 `#0F7C86` 쌍 — CVD ΔE 8.7·정상시 26.0·대비 ≥3:1 PASS, 영도 채도(0.089) 만 «회색으로 읽힘» FAIL → 정체성 색이라 유지하고 2차 인코딩(끝 라벨·범례·툴팁·표 뷰)으로 보완. 선 차트 «한 축» 규칙은 인용·방문자를 **타일 둘로 분리**해서 지켰다(단위가 달라 한 축에 못 올린다).
