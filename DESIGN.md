...
- 스냅샷 생산 = **데스크 [E](매일 06:00) 하루 1회뿐**(사용자 결정 09-05 — 드레이너 발행마다 갱신은 철회). 타일 머리는 «n분 전»이 아니라 스냅샷 시각(`06:12 기준`, 다른 날이면 `9.4 06:12 기준`)을 보인다. 토큰 만료면 `quota.cjs` 는 보류(exit 0)하고 마지막 값을 그 시각과 함께 보여 준다 — 0으로 위장하지 않는다. «일간 발행»·«작업 중»은 스냅샷이 아니라 백엔드 실시간(1분 재조회)이다.

### Request Modal — 새 글 요청·발주 요청 (2026-09-05 정리)
- **칸 4개뿐**: 주제 · 요청 사항(텍스트 영역 6줄 = `min-height:164px`, 스크롤 없이) · 작성자 · 요청 문서. 「글의 목적」은 묻지 않는다(주제에 담긴다 — 백엔드 `purpose=null`, 러너가 추정. 트렌드 발주만 데스크 purpose 를 조용히 동봉). 절 표기(필수/선택)·헤더 부제·작성자 안내문·첨부 안내 3줄 전부 제거 — 남는 문구는 라벨 4개 + 드롭존 안 두 줄(«파일을 여기에 놓거나 클릭» / «형식 · 10MB · 이 요청 1건에만 최우선 규칙으로 적용»)뿐.
- **요청 문서 드롭존 `.dz`**(09-05): 면(`surface-2`)+헤어라인 사각 영역 하나. 클릭·Enter/Space → 파일 선택창, 드래그 앤 드롭 → 그 자리에서 형식(docx·xlsx·pdf·txt·hwpx)·10MB 검증 후 파일명·크기로 바뀌고 «지우기». 놓기 직전 `.over` = 잉크 2px 테두리·1% 확대·아이콘 2px 상승. 여러 개 놓으면 첫 파일만(토스트). 모달 전체에서 드롭 기본 동작(파일 열림) 차단.
- **작성자는 사람이 고른다**: 「자동 배정」 없음, 기본 선택 없음. 미선택 전송 → «작성자를 선택해주세요» + 칩 무리 경고 테두리 1.8초 + 첫 칩 포커스. 트렌드 발주는 데스크 lane 이 있으면 기본 선택.
- **요청 문서 = 광고주 가이드**: 1회용·그 글의 최우선 규칙(첨부 > 문체·목적 규칙, 단 발행 게이트 금지요소·공식 사실 충돌은 따르지 않고 failed+사유 환류). 키보드로도 첨부 가능(`.f-fbtn` tabindex+Enter/Space).
