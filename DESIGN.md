...
- **게이트** `_tools/verify-toolbox.mjs`(썸네일) + `_tools/verify-tier.mjs`(티어표, 09-07) — 둘 다 헤드리스 CDP · 의존성 0. 썸네일 쪽(v2.1 95항목: 탭 진입·로드·이미지 등록·타이틀 카운터·프리셋 5·긴 제목 축소·부제 경고·색·글꼴·확대·드래그·가운데·PNG 파일명·blob·미리보기 실크기 183/90/모바일 3열·Esc·포커스 복귀·탭 이탈/복귀 상태 유지·390/360 섬 넘침·종 겹침·모달 축소 배율) + design-reviewer 실렌더.
