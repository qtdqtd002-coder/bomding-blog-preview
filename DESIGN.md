...
- **칸 4개뿐**: 주제 · 소재·참고 메모 · 작성자 · 요청 문서. 「글의 목적」은 묻지 않는다(주제에 담긴다 — 백엔드 `purpose=null`, 러너가 추정. 트렌드 발주만 데스크 purpose 를 조용히 동봉). 절 표기(필수/선택)·헤더 부제·작성자 안내문·첨부 안내 3줄 전부 제거 — 남는 문구는 라벨 4개 + 힌트 1줄(«이 요청 1건에만 최우선 규칙으로 적용 · 형식 · 10MB»)뿐.
