...
### Topic Desk Category (주제 분류 전용 — 2026-08-14 신설)
트렌드 탭의 일간 토픽 데스크가 쓰는 **네 번째 색 축**이다. 작성자 색과 목적이 다르다 —
저건 "누가 썼나", 이건 "무슨 종류의 소식인가".

- 신작 게임 `--c-new` `#1B5FA8` · 최근 업데이트 `--c-upd` `#15703C` ·
  최근 화제 `--c-hot` `#C4482A` · 콘솔 게임 `--c-con` `#5B4B8A`

**왜 새 축을 허용했나.** 앞의 셋은 이미 대비 검증을 마친 기존 토큰(`info`·`ok`·`accent`)을 그대로 재사용하므로
실제로 늘어난 색은 **콘솔 바이올렛 하나뿐**이다(종이 위 6.5:1, 다크 `#A99AE0` 7.6:1 — 양쪽 AA).
그리고 트렌드 탭에는 **작성자 색이 한 픽셀도 렌더되지 않는다**(에디션에 writer 필드가 없다) —
두 축이 같은 화면에서 만나는 곳은 발주 모달의 작성자 칩뿐이고, 거기선 분류색이 헤더 아이콘 1개로만 나온다.

**The Category-Never-Alone Rule.** 분류는 **색만으로 전달하지 않는다.**
섹션 헤더는 항상 `아이콘 + 한글 라벨 + 건수`를 함께 띄우고, 카드의 분류 정보도 게임 칩·목적 태그 텍스트로 중복 표기한다.
작성자 색에 적용한 규칙(색 옆에 항상 이름)을 그대로 따른다.
...
- **뷰 전환**: 나갈 때 160ms 접힘 → 들어올 때 340ms 아래에서 올라옴(`withTransition`). 탭·작성자·페이저 이동에만 적용하고, 정렬 같은 제자리 갱신에는 쓰지 않는다.
- **세그먼트 인디케이터**: 선택 표시가 탭 사이를 340ms 미끄러진다 — 어디로 옮겨갔는지 눈이 따라간다.
- **글 진입**: 화면이 210ms 물러난 뒤 이동. `pageshow`에서 반드시 원복(뒤로가기로 돌아왔을 때 흰 화면 고착 방지).
