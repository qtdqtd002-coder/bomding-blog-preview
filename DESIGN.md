...
★**09-05 2차 개편(사용자 결정)** — 잔여 사용량은 타일에서 빠져 **상단 고정 칩**(종 왼쪽, 모든 탭)이 됐고, 주간 사용량+일간 발행은 **「사용량」 한 차트**로 합쳤으며, 일별 방문자 타일은 뺐다(로거는 계속 기록). 남은 타일 = 작업 중 · 사용량 · AI 브리핑 인용.
...
| **AI 브리핑 인용** (12) — 09-05 | `_trend/_citations.json`(`_tools/mate-citations.cjs`, 네이버 공개 프로필 일별 스냅샷) | **작성자 2칸**(`.cr-grid` 봄딩 ｜ 영도, 헤어라인 세로 구분 · 860px 이하 세로 스택): 점+이름+«9월 게임 메이트» 칩 → 스탯 4칸(어제 증분·이달·기준월·누적, 모노 숫자) → 14일 단일 시리즈 막대(작성자 색, ≤24px, 캡 라벨=최대·마지막만, 툴팁=증분·누적·스냅샷 시각, 표 뷰). **어제까지만 값이 있고 오늘은 «집계 전»**. 인용수는 메이트 선정 블로그만 공개라 영도는 «인용수 비공개 · 메이트 선정 시 자동 공개»(0 으로 그리지 않는다) |
...
- **과도기 상태(09-05)**: 두 타일은 «한쪽만 있음·전무»를 정상 상태로 그린다 — 인용 증분이 없으면 스탯 4칸(네이버 원값)만 두고 «일별 증분 집계 전 · 스냅샷 N개», 인용수 비공개(메이트 선정 전)면 «인용수 비공개 · 메이트 선정 시 자동 공개» 1줄. 빈 자리를 0이나 장식으로 채우지 않는다. 키보드 포커스 툴팁은 열 «옆»에 붙는다(위에 올리면 타일 헤더와 겹침).
...
- 스냅샷 생산 = **데스크 [E](매일 06:00) 하루 1회뿐**(사용자 결정 09-05 — 드레이너 발행마다 갱신은 철회). 타일 머리는 «n분 전»이 아니라 스냅샷 시각(`06:12 기준`, 다른 날이면 `9.4 06:12 기준`)을 보인다. 토큰 만료면 `quota.cjs` 는 보류(exit 0)하고 마지막 값을 그 시각과 함께 보여 준다 — 0으로 위장하지 않는다. «작업 중»과 「사용량」의 발행 편수는 스냅샷이 아니라 백엔드 실시간(1분 재조회)이다.
